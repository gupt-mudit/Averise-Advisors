...
export const Navbar = () => {
    ...
    return (
        <header className="flex items-center justify-between -mt-12 px-12 md:px-32 drop-shadow-md">
            ...
            {isMenuOpen ? (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6" onClick={() => setIsMenuOpen(!isMenuOpen)}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="m9.75 9.75 4.5 4.5m0-4.5-4.5 4.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
                </svg>
            ) : (
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="size-6 xl:hidden container z-]" onClick={() => setIsMenuOpen(!isMenuOpen)}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/>
                </svg>
            )}

            {isMenuOpen && (
                <div className="fixed right-0 z-20 top-24 border-8 rounded-md w-full p-12 my-24 flex flex-col justify-center items-center xl:hidden glass-effect-nav">
                    <ul>
                        <li className="py-2"><HashLink to="/#home" onClick={() => setIsMenuOpen(false)}>home</HashLink></li>
                        <li className="py-2"><HashLink to="/#about" onClick={() => setIsMenuOpen(false)}>about</HashLink></li>
                        <li className="py-2"><HashLink to="/#articles" onClick={() => setIsMenuOpen(false)}>articles</HashLink></li>
                        <li className="py-2"><HashLink to="/#testimonials" onClick={() => setIsMenuOpen(false)}>testimonials</HashLink></li>
                        <li className="py-2"><HashLink to="/#contact" onClick={() => setIsMenuOpen(false)}>contact</HashLink></li>
                    </ul>
                </div>
            )}
        </header>
